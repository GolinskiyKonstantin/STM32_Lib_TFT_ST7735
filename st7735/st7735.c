	...
	//-- если захотим переделать под CMSIS  ---------------------------------------------
		...
		//======  FOR F-SERIES ===========================================================
			...
			*((__IO uint8_t *)&ST7735_SPI_CMSIS->DR) = Cmd;
			...
			while(!(ST7735_SPI_CMSIS->SR & SPI_SR_TXE)){};
...
char ST7735_DrawChar(uint16_t x, uint16_t y, uint16_t TextColor, uint16_t BgColor, uint8_t TransparentBg, FontDef_t* Font, uint8_t multiplier, unsigned char ch){
	...
	if (
		ST7735_Width <= ( x + Font->FontWidth) || ST7735_Height <= ( y + Font->FontHeight)){
		/* Error */
		return 0;
	}
...
char ST7735_print(uint16_t x, uint16_t y, uint16_t TextColor, uint16_t BgColor, uint8_t TransparentBg, FontDef_t* Font, uint8_t multiplier, char *str){	
	...
	while (*str) {
		...
		if ( ST7735_DrawChar(x, y, TextColor, BgColor, TransparentBg, Font, multiplier, *str) != *str ){
			/* Return error */
			return *str;
		}
